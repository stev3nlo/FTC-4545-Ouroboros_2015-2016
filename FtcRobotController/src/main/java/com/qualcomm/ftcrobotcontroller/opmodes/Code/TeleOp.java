...
public class TeleOp extends OpMode {
    ...
    public void loop() {
        ...
        if(gamepad1.a){
                if(reverse){
                    reverse = false;if (gamepad1.left_trigger == 1) {
                        if (abs(gamepad1.right_stick_y) > .05) { //Half speed for drive
                    ...
                    if (gamepad2.left_bumper) { //Tilt the box
                        if (boxTiltPosition != 0) {
                            boxTiltPosition -= .05;
                            boxTilt.setPosition(boxTiltPosition * -1); //While left bumper is pushed, subtract .05 from the position of the servo
                        }
                    }
                    if (gamepad2.right_bumper) { //Tilt the box in the other direction
                        if (boxTiltPosition != 1) {
                            boxTiltPosition += .05;
                            boxTilt.setPosition(boxTiltPosition *-1); //While right bumper is pushed, add .05 from the position of the servo
                        }
                    }
